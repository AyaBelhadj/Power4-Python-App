...
titre=Frame(windows, bg='#ECD7DF')
#ajoute un titre dans la fenetre
windows.title("Jeux puissance 4 ")
windows.geometry("700x700")
windows.maxsize(750,750)
windows.minsize(750, 750)
windows.iconbitmap("icon.ico")
windows.config(background='#8cd3fb')

#creation de canavas=surface permettant de dessiner des formes geo
des = Canvas(windows, width=500, height=425,bg='#8cd3fb',bd=0,highlightthickness=0)

#frame= fonctionne comme un conteneur. C’est une zone rectangulaire dans laquelle des widgets peuvent être placés.
b=Frame(windows, bg='#8cd3fb')
Noms=Frame(b, bg='#8cd3fb')
# inserer le titre
#label=afficher un texte ou une image à l’écran. Label ne peut afficher du texte que dans une seule police. Le texte affiché par ce widget peut être mis à jour à tout moment.
titre1= Label(b, text="Puissance 4\n", font=("Arial Rounded MT Bold",55), bg='#8cd3fb', fg='#f0763f',bd=0)
titre1.grid(row=0, column=0, padx=150, pady=50,ipadx=0,ipady=0)
#inserer les noms des joueurs
#joueur 1
label1=Label(Noms,text="Player 1 :",font=("Arial Rounded MT Bold",18), bg='#8cd3fb', fg='#f0763f',bd=0)
label1.grid(row=2, column=1, padx=0, pady=15, ipadx=0, ipady=0)
nom1=Entry(Noms,width=20,font=("courrier",15))
nom1.grid(row=2, column=2, padx=0, pady=0, ipadx=0, ipady=0)
#joueur 2
label2=Label(Noms,text="Player 2 :",font=("Arial Rounded MT Bold",18), bg='#8cd3fb', fg='#f0763f',bd=5)
label2.grid(row=4, column=1, padx=10, pady=15, ipadx=0, ipady=0)
nom2=Entry(Noms,width=20,font=("courrier",15))
nom2.grid(row=4, column=2, padx=10, pady=0, ipadx=0, ipady=0)
Noms.grid()
#inserer la bouton JOUER
play=Button(b,text="Start !!",height=2, width=30, bg='white', fg='#f0763f', font=("Arial Rounded MT Bold", 20), command=lambda:puissance4())
...
def puissance4():
    ...
    joueur1 = nom1.get()
    joueur2 = nom2.get()
    #supprimer le contenue de page principale
    play.destroy()
    b.destroy()
    #insertion du titre de jeu avec le nom du joueur qui  va jouer
    titre1 = Label(windows, text= joueur1+" it's your turn !", font=("Arial Rounded MT Bold", 30), bg='#8cd3fb',fg='#f0763f', bd=5)
    ...
    def Fctbouton(id):
        ...
        if (ActivePlayer==1):
            ...
            titre1 = Label(windows, text= joueur2+" it's your turn !", font=("Arial Rounded MT Bold", 30), bg='#8cd3fb', fg='#f0763f', bd=5)
            titre1.grid(row=0, column=0, padx=0, pady=12, ipadx=0, ipady=0)
            Creercercle(id,'#ff6634')
    ...
    def gagnant():
        if ((win_vertical() == 1) or (win_horizontal() == 1) or (win_diagonale() == 1)):
            messagebox.showinfo(title='Result', message=joueur1 + ' is the winner ')
        elif ((win_vertical() == 2) or (win_horizontal() == 2) or (win_diagonale() == 2)):
            messagebox.showinfo(title='Result', message=joueur2 + ' is the winner ')
            ...
            messagebox.showinfo(title='Result', message='NULL')
    ...
    f_m.add_command(label="Quitter", command=windows.destroy)

    menu.add_cascade(label="Menu", menu=f_m)
